...
**v1 funcionalmente completo — todos os 5 componentes implementados, incluindo resiliência real sem banco e entrada via URL de repositório Git.** Scanner, Analyzer, Persistence, Exporter e CLI (`com.arqsync.cli` + `com.arqsync.ArqSyncApplication`) estão implementados e testados (`./mvnw test` verde, 91/91 testes Java, cobertura acima do gate de 70%; `pytest` verde, 7/7 testes Python). **Pipeline validado de ponta a ponta de verdade** nesta sessão em dois cenários contra um PostgreSQL real (instalado localmente, não Docker): banco disponível (persistência real acontece) e banco indisponível (aplicação sobe mesmo assim, sem persistência, gerando `report.json`/`report.html` normalmente). O CLI agora também aceita uma URL de repositório Git (`http://`/`https://`) como argumento único, clonando-a via JGit antes do scan — validado com clone real de um repositório público (`GitRepositoryResolverIT`). `./mvnw verify` só é validável fora deste ambiente de sessão para o `PersistenceIT` (Testcontainers/Docker) — ver Pendências.
...
| CLI (`com.arqsync.cli` + `com.arqsync.ArqSyncApplication`) | ✅ Implementado, testado, validado ponta a ponta com Postgres real; aceita caminho local ou URL Git (`GitRepositoryResolver`, JGit) | [`src/main/java/com/arqsync/cli`](src/main/java/com/arqsync/cli) |
...
- **Entrada via URL de repositório Git — implementada nesta sessão.** O argumento único do CLI agora aceita, além de um caminho local, uma URL `http://`/`https://` de repositório Git (SPEC-cli.md, 2.11). `GitRepositoryResolver` (novo, `com.arqsync.cli`) valida a URL, clona com JGit (`org.eclipse.jgit:org.eclipse.jgit:6.10.0.202406032230-r`) para um diretório temporário (`Files.createTempDirectory("arqsync-clone-")`, timeout de clone de 5 minutos), e aplica os limites do v1: apenas repositórios públicos, até 100MB de clone, até 10.000 arquivos `.java`. `ArqSyncPipelineRunner` executa clone+scan sob um orçamento total de 10 minutos (via `ExecutorService`/`Future.get(..., TimeUnit.MINUTES)`), roda o pipeline normal sobre o diretório clonado, e remove o diretório temporário ao final em um bloco `finally` — garantindo limpeza tanto em sucesso quanto em qualquer falha fatal subsequente (Analyzer/Persistence/Exporter). Nova flag `--keep` preserva o diretório clonado para depuração. Validado com clone real de um repositório público (`com.arqsync.cli.GitRepositoryResolverIT`, Failsafe) e com testes determinísticos para os cenários de erro (`com.arqsync.cli.GitRepositoryResolverTest`): URL inválida (validação de sintaxe, sem tocar rede), timeout de clone (servidor TCP local que nunca responde, combinado com um timeout curto injetado via construtor de teste) e limites de tamanho/contagem de arquivos excedidos (simulados com arquivo esparso e diretórios com poucos arquivos, sem precisar de um clone real de 100MB/10.000 arquivos). `./mvnw test`: 91/91 verdes; `./mvnw verify` inclui o novo `GitRepositoryResolverIT` (2/2 verdes, rede real) além do `PersistenceIT` já conhecido (falha só por falta de Docker neste ambiente, não relacionado a esta mudança).
